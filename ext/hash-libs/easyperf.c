...
static int *fds = NULL;
static unsigned int n_counters;
...
int perf_init(unsigned int num_ctrs, ...) {
    ...
    for (ctr=0; ctr<num_ctrs; ctr++) {
		...
		attr[ctr].type = PERF_TYPE_RAW;	// type of event
		attr[ctr].config = event; /* generic PMU event*/ // event code
		attr[ctr].config1 = 3;	// ldlat=3
		attr[ctr].disabled = 0;	
		attr[ctr].pinned = 1;
		attr[ctr].inherit = 1;
		fds[ctr] = perf_event_open(&attr[ctr], getpid(), -1, -1, 0);	// initiate the register of the event
...
void perf_close() {
	...
	for (i=0; i<n_counters; i++) {
		close(fds[i]);	// close the file descriptor of the registration of the event
	}
...
uint64_t perf_read(unsigned int ctr) {
	uint64_t val;
	int rc;
	assert(ctr < n_counters);	// check if the index of the counter is valid
	rc = read(fds[ctr], &val, sizeof(val));	// read the value of the counter
	assert(rc == sizeof(val));
	return val;	// return the value of the counter
}

void perf_read_all(uint64_t* vals) {
	unsigned int i;
	for (i=0; i<n_counters; i++) {	// read the value of each counter and store it in the array
		vals[i] = perf_read(i);
	}
}
